...
def optimize_crf_with_bitrate_cap(original_video, initial_crf=23, min_crf=0, max_crf=51, max_iterations=10, vmaf_model_path=None):
    lower_bound = min_crf
    upper_bound = max_crf
    crf = initial_crf
    optimal_crf = None
    ...
    for iteration in range(max_iterations):
        ...
        # Check if the quality metrics meet the thresholds and bitrate is under the limit
        if (vmaf_score >= VMAF_THRESHOLD and psnr_score >= PSNR_THRESHOLD and ssim_score >= SSIM_THRESHOLD and bitrate <= BITRATE_THRESHOLD):
            optimal_crf = crf
    ...
    if optimal_crf is not None:
        print(f"Optimal CRF found: {optimal_crf} with bitrate under {BITRATE_THRESHOLD} kbps")
    else:
...
# Example usage with bitrate cap
